...
{
    public partial class fQLYT : Form
    {
        CanBoYTe nv = new CanBoYTe();
        Boolean themmoi;
        public fQLYT()
        {
            InitializeComponent();
        ...
        {
            lsvNV.Items.Clear();
            DataTable dt = nv.timkiemTenCanBo(txtTenhoacMa.Text);
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ListViewItem lvi =
                lsvNV.Items.Add(dt.Rows[i][0].ToString());
        ...
        {
            lsvNV.Items.Clear();
            DataTable dt = nv.timkiemMaCanBo(txtTenhoacMa.Text);
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ListViewItem lvi =
                lsvNV.Items.Add(dt.Rows[i][0].ToString());
        ...
        private void fQLYT_Load(object sender, EventArgs e)
        {
            HienthiCanBo();
            setnull();
            setKhoa(true);
        ...
        private void lsvNV_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (lsvNV.SelectedIndices.Count > 0)
            {
                ...
                txtChuyenMon.Text = lsvNV.SelectedItems[0].SubItems[6].Text;               
        ...
        private void spbXoa_Click(object sender, EventArgs e)
        {
            if (lsvNV.SelectedItems.Count > 0)
            {
                ...
                try
                {
                    ...
                    {
                        nv.XoaCanBo(lsvNV.SelectedItems[0].SubItems[0].Text);
                ...
                {
                    MessageBox.Show("Không thể xóa cán bộ y tế này");
        ...
        private void spbLuu_Click(object sender, EventArgs e)
        {
            ...
            if (themmoi)
            {
                ...
                {
                    MessageBox.Show("Trùng mã cán bộ y tế");

                }
                else
                {
                    nv.ThemCB(txtMaNV.Text, txtTenNV.Text, ngay, cmbGioiTinh.Text, txtSDT.Text, txtTrinhDo.Text,txtChuyenMon.Text, txtDiaChi.Text, txtEmail.Text);
                    MessageBox.Show("Thêm mới thành công");
                }
            }
            else
            {
                nv.CapNhatCB(lsvNV.SelectedItems[0].SubItems[0].Text, txtTenNV.Text, ngay, cmbGioiTinh.Text, txtSDT.Text, txtTrinhDo.Text, txtChuyenMon.Text, txtDiaChi.Text, txtEmail.Text);
